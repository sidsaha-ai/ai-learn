...
def test_indexing() -> None:
    ...
    # select one row
    emb_slice = emb[0]
    assert emb_slice.shape[0] == embedding_dim

    # select one item
    emb_slice = emb[0, 1]
    assert isinstance(emb_slice.item(), float)

    emb_slice = emb[0, [1, 2, 4]]
    assert emb_slice.shape[0] == 3

    # select rows 0 and 1, and colummns 1, 2, and 4
    emb_slice = emb[[0, 1], :][:, [1, 2, 4]]
    assert emb_slice.shape == (2, 3)
